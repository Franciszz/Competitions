...
class ApplResample(BaseTransformer):
    ...
    def train_test_split(self):
        ...
        m, n, k = len(df_0), len(df_1), len(df_test)
        np.random.seed(self.seed)
        m_eval_index = np.random.permutation(m)
        np.random.seed(self.seed)
        n_eval_index = np.random.permutation(n)
        
        df_0_eval = df_0.iloc[m_eval_index[:int(k*m/(m+n))],:]
        df_defaultless = df_0.iloc[m_eval_index[int(k*m/(m+n)):],:]
        
        df_1_eval = df_1.iloc[n_eval_index[:int(k*n/(m+n))],:]
        df_default = df_1.iloc[n_eval_index[int(k*n/(m+n)):],:]
