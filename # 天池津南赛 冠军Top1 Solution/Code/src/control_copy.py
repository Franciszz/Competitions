...
def feature_engineer_materials(data, na_value=405):
    ...
    for _f in _fs[:-2]:
        ...
        # df[f'{_f}_P5_O_4M_R'] = df['P5_O_4M'] / df[_f]
...
for i in tqdm.tqdm(range(len(df_seed))):
    ...
    for _f in tst.columns.tolist()[1:-1]:
        ...
        # if len(vc) <= 12 and vc.iloc[0] <= 0.75:
        #     recall_mean = trn.groupby([_f])['收率'].mean()
        #     for _df in [trn, tst]:
...
# pred_control['pred'].reset_index().to_csv(
#     'submit/submit_control_lst.csv', header=None, index=False)