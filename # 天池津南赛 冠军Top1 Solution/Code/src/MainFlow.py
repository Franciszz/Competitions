...
for _f in df_tst.columns.tolist()[1:]:
    ...
    if len(vc) <= 12 and vc.iloc[0] <= 0.4:
...
for _f in [f'recall_{i}' for i in range(1, 5)]:
    del df_trn[_f]
del _f, vc, corr, recall_count, _idx_col, _df, _col #, recall_mean
# for _df in [df_trn, df_tst]:
...
params_lgb = {'num_leaves': 120, 'max_depth': 7, 'learning_rate': 0.025,
              'min_data_in_leaf': 30,  # 'min_child_samples': 45,
...
fit_params = {'num_boost_round': 10800,
              'verbose_eval': False,
              'early_stopping_rounds': 360}
params_xgb = {'eta': 0.01, 'max_depth': 7, 'subsample': 0.723,
...
pred_xgb, pred_trn_xgb = xgb_cv(
    ...
    df_trn.columns.tolist()[1:-1], 5, 19960101)
