...
def duration_cal_tran(data):
    ...
    for _col_a, _col_b in zip(cols_timer[::-1][:-1], cols_timer[::-1][1:]):
        df[f'{_col_a}_diff'] = duration_outer(df[_col_a], df_temp[_col_b])
    del _col_a, _col_b
    return df#.reindex(columns=df_trn_tst.columns)


df_trn = duration_cal_tran(df_trn)
df_tst = duration_cal_tran(df_tst)
df_trn_tst = duration_cal_tran(df_trn_tst)

# ====== EDA ========
with sns.axes_style('white'):
    trn['收率'].hist(bins=30, color='#4682B4')
    plt.title('Dist. of Recall Rate')
with sns.axes_style('white'):
    plt.scatter(range(trn.shape[0]), np.sort(trn['收率'].values),
                c='#4682B4', s=2)
df_corr = df_trn.iloc[:, 1:].corrwith(trn['收率']).abs().\
    sort_values(ascending=False).head(32)
plt.figure(figsize=(10, 7.2))
with sns.axes_style('white'):
    sns.barplot(y = df_corr.index, x = df_corr, color = '#4682B4')
with sns.axes_style('white'):
    plt.scatter(range(df_trn.shape[0]), np.sort(df_trn['B14'].values),
                c='#4682B4', s=2)
    plt.ylim((0, 800))
with sns.axes_style('white'):
    plt.scatter(range(df_tst.shape[0]), np.sort(df_tst['B14'].values),
                c='#B22222', s=2)
    plt.ylim((0, 800))
df_trn['recall'] = df_target['收率']
df_trn = df_trn.query('recall > 0.7')
with sns.axes_style('white'):
    sns.jointplot(df_trn['B14'], df_trn['recall'], color='#4682B4')
    plt.xlabel('B14')
...
def NonparametricScreening(X, y, quant=None, knots=None, seed=2019):
    n, p = X.shape
    if knots is None:
        knots = np.ceil(n**0.2)
    if quant is None:
        quant = 1
    knots_df = knots + 1
    X_bs = np.zeros((n, int(p*knots_df)))
    y_mix = y.sample(n, replace=True,random_state=seed).reset_index(drop=True)
    ori_mix_res = np.zeros((p, 2))
    for i in range(p):
        X_bs = dmatrix(
            'bs(x, df = df, include_intercept = False)',
            {'x': X.iloc[:, i], 'df': knots_df},
            return_type = 'dataframe')
        lm_ori = sm.OLS(y, X_bs).fit()
        lm_mix = sm.OLS(y_mix, X_bs).fit()
        ori_mix_res[i, 0] = np.round(np.mean(np.sqrt(np.square(lm_ori.resid))),4)
        ori_mix_res[i, 1] = np.round(np.mean(np.sqrt(np.square(lm_mix.resid))),4)
    sort_ori_res_idx = np.argsort(ori_mix_res[:, 0])
    thr_res = np.percentile(ori_mix_res[:, 1], 1 - quant)
    nsis = min(np.sum(ori_mix_res[:, 0] < thr_res), n)
    print('Number of coefficients left is:', nsis)
    SISind = np.sort(np.array(sort_ori_res_idx)[:nsis], axis=0)
    X_new = X.iloc[:, np.sort(sort_ori_res_idx[:int(nsis)])]
    lasso_reg = LassoCV()
    newSISind = np.nonzero(np.abs(lasso_reg.fit(X_new, y).coef_))[0]
    print('Number of coefficients left via LassoCV:', len(newSISind))
    X_lst = X_new.iloc[:, newSISind]
    print(SISind[newSISind])
    return X_lst

# ====== Conduct the lasso cv ==================================================
X_lst = NonparametricScreening(
    df_trn.iloc[:, 1:-1], df_trn['收率'], quant=0.75, seed=19960101)
...
def lgb_cv(train, test, params, fit_params,
           cat_features, feature_names, nfold, seed):
    model_FI = pd.DataFrame(index=feature_names)
    train.Pred = pd.DataFrame({
        'id': train['样本id'],
        'true': train['收率'],
        'pred': np.zeros(len(train))})
    ...
    for fold_id, (trn_idx, val_idx) in enumerate(kfolder.split(train['收率'])):
        ...
        val_pred = lgb_reg.predict(
            train.iloc[val_idx][feature_names],
            num_iteration=lgb_reg.best_iteration)
        model_FI['fold' + str(fold_id)] = \
            lgb_reg.feature_importance() / lgb_reg.feature_importance().sum()
        train.Pred.loc[val_idx, 'pred'] = val_pred
        print(f'Fold_{fold_id}', mse(train.iloc[val_idx]['收率'], val_pred))
        test.Pred['pred'] += lgb_reg.predict(
            test[feature_names], num_iteration=lgb_reg.best_iteration) / nfold
    print('\n\nCV LOSS:', mse(train.Pred['true'], train.Pred['pred']))
    return test.Pred, train.Pred, model_FI

pred_lgb, pred_trn_lgb, model_fi = lgb_cv(
    df_trn, df_tst, params_lgb, fit_params,
    cat_features, df_trn.columns.tolist()[1:-1], 4, 19960101)

model_fi['imp'] = model_fi.sum(axis=1)
model_fi.sort_values(['imp'], ascending=False, inplace=True)
model_fi = model_fi.head(30)
plt.figure(figsize=(10,8))
with sns.axes_style('white'):
    sns.barplot(y = model_fi.index, x = model_fi['imp'], color = '#B22222')

df_target = df_trn[['样本id', '收率']]
df_target['pred_lgb'] = df_trn_pred_2
df_target['pred_nis'] = df_trn_pred_1
df_target.sort_values(['收率'], inplace=True)
with sns.axes_style('white'):
    plt.figure(figsize=(12,8))
    plt.scatter(df_target['收率'], df_target['pred_lgb'],
                s=3, label='lgb:0.00135', c = '#4682B4')
    plt.scatter(df_target['收率'], df_target['pred_nis'],
                s=3,label='nis:0.00152', c = '#B22222')
    plt.legend()
