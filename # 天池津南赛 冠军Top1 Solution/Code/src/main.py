...
df_tst = test_abnormal_revise(df_tst).pipe(test_manual_adjust)
...
def feature_engineer_materials(data, na_value=405):
    ...
    df['P5_O_5M'] = raw['B14'].replace(418, 420).replace(405, 400).\
        replace(395, 400).replace(392, 390).replace(387, 390).\
        replace(385, 380).replace(370, 380).replace(360, 380).\
        replace(350, 380).replace(340, 280).replace(290, 280).\
...
def train_duplicated_mode(data):
    df, len_df = data.copy(), len(data)
...
for _f in df_tst.columns.tolist()[1:-1]:
    ...
    if len(vc) <= 18 and vc.iloc[0] <= 0.6:
    ...
    if len(vc) <= 20 and corr >= 0.36 and vc.iloc[0] <= 0.75:
...
del _df, df_feature
gc.collect()
del _f, vc, _idx_col, _col, recall_count, corr
gc.collect()
...
pred_xgb, pred_trn_xgb = xgb_cv(
    ...
    df_trn.columns.tolist()[1:-1], 5, 19950520)
pred_xgb.to_csv(f'submit/submit_{datetime.now().strftime("%m%d%H%M")}.csv',
