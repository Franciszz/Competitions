...
def make_train(train, test, *train_add):
    ...
    df_seed = df_seed[(df_seed['收率'] <= 0.935) & (df_seed['收率'] >= 0.912)]
...
def make_prediction(train, test, sed):
    ...
    for i in range(len(df_seed)):
        print(f'# ====== record {i}/{len(df_seed)} training ...')
        trn, tst = df_trn.copy(), df_tst.copy()
        trn.iloc[:, 2:-1] = \
...
if __name__ == '__main__':
    ...
    df_tst_c = pd.read_csv(
        'data/jinnan_round1_test_20190121.csv', encoding='GB2312')
    ...
    df_trn_1, df_tst_1, df_seed_1 = make_train(df_trn, df_tst_a)
    pred_xgb_a = xgb_cv(
        df_trn_1, df_tst_1, params_xgb, fit_params,
        df_trn_1.columns.tolist()[1:-1], 5, 19950520)
    df_tst_a['收率'] = pred_xgb_a['pred'].values
    df_trn_2, df_tst_2, df_seed_2 = make_train(
        df_trn, df_tst_b, df_tst_a)
    pred_xgb_b = make_prediction(
        df_trn_2, df_tst_2, df_seed_2)
    pred_xgb_b.to_csv('submit_B.csv', index=False, header=None)
    df_tst_b['收率'] = pred_xgb_b['pred'].values
    df_trn_3, df_tst_3, df_seed_3 = make_train(
        df_trn, df_tst_c, df_tst_a, df_tst_b)
    pred_xgb_c = make_prediction(df_trn_3, df_tst_3, df_seed_3)
    pred_xgb_c.to_csv('submit_C.csv', index=False, header=None)